...
class Menu {
    ...
    string ingameINSTRUCTIONS[5] = {">>INGAME INSTRUCTIONS<<", "W TO JUMP", "A/D to move [DURING BOSS FIGHT ONLY]", "IF GAME OVER, PRESS O TO RESTART", "CHEAT: PRESS O WHEN NOT DEAD TO TELEPORT TO BOSS ROOM"};
    ...
    void menuOpen() { 
        ...
        for (int i = 0; i < 5; i++) {
            ...
            // Printing menu movement instructions
            if (i < 3) {
                console.gotoxy(50, 3 + i);
                cout << "\033[32m\033[4m" << instructions[i] << "\033[0m";

                console.gotoxy(cursorX, cursorY + i);
                cout << options[i];
            }

            //Printing ingame instructions
            console.gotoxy(50, 7 + i);
            cout << "\033[32m\033[4m" << ingameINSTRUCTIONS[i] << "\033[0m";
